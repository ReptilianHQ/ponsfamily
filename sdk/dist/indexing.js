...
export const ponsIndexingAbis = deepFreeze(catalogAbis);
const address = (description) => ({ unit: "address", description });
const identifier = (description) => ({ unit: "identifier", description });
const amount = (asset, description) => ({
...
const quote = (description) => amount("launch pairToken (pool quoteToken for hook events)", description);
...
export const ponsIndexingCatalog = deepFreeze(Object.entries(catalogAbis).flatMap(([contract, abi]) => {
    const descriptions = metadata[contract];
    return abi.filter((item) => item.type === "event")
        ...
        if (!info)
            throw new PonsSdkError("INVALID_ARGUMENT", `Missing indexing metadata for ${contract}.${event.name}`);
        return {
            contract: contract, name: event.name,
            ...
            parameters: event.inputs.map(input => {
                ...
                if (!semantic)
                    throw new PonsSdkError("INVALID_ARGUMENT", `Missing indexing semantic for ${contract}.${event.name}.${input.name}`);
                return { name: input.name, type: input.type, indexed: "indexed" in input && input.indexed === true, semantic };
            }),
        };
    });
}));
const EVENTS = Object.fromEntries(Object.keys(ponsIndexingAbis).filter(name => name !== "UniswapV4PoolManager")
    .map(name => [name, ponsIndexingCatalog.filter(event => event.contract === name).map(event => event.name)]));
const param = (parameter, lowercase = false) => ({ parameter, lowercase });
const block = { event: "block.number" };
const source = { event: "srcAddress", lowercase: true };
export const ponsIndexingMaterializations = deepFreeze([
    {
