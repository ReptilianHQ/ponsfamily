...
export function getAmountOut(
  ...
  const denominator = checkedAdd(
    checkedMul(reserveIn, BASIS_POINTS, "amountOut.reserveInScaled"),
    amountInWithFee,
...
export function getAmountIn(
  ...
  assertPositive(reserveOut, "reserveOut");
  assertBigintBps(feeBps, "feeBps");
  if (amountOut >= reserveOut) invalid("amountOut", "less than reserveOut", amountOut);
  const numerator = checkedMul(
    checkedMul(amountOut, reserveIn, "amountIn.outputReserveProduct"),
    BASIS_POINTS,
    "amountIn.numerator",
  );
...
export function quoteCurveBuy(
  parameters: CurveQuoteParameters & { sellableTokens: bigint },
): bigint {
 ...
 */
export function quoteCurveBuyExecution(
  parameters: CurveQuoteParameters & { sellableTokens: bigint },
  ...
  if (tokensOut > parameters.sellableTokens) {
    ...
    const grossedUp = mulDivCeil(
      requiredNet,
      BASIS_POINTS,
      BASIS_POINTS - parameters.feeBps - parameters.creatorTaxBps,
    );
...
export function quoteCurveBuyExactTokensOut(parameters: ExactTokenBuyParameters): CurveBuyExecutionQuote {
  totalTradeFee(parameters.feeBps, parameters.creatorTaxBps);
  ...
  while (low < high) {
    ...
    if (quoted.tokensOut >= parameters.tokensOut) high = midpoint;
    else low = midpoint + 1n;
  }
  return quoteCurveBuyExecution({ ...parameters, amountIn: low });
}

export function quoteCurveSell(parameters: CurveQuoteParameters): bigint {
...
function totalTradeFee(feeBps: bigint, creatorTaxBps: bigint): bigint {
  assertBigintBps(feeBps, "feeBps");
  assertBigintBps(creatorTaxBps, "creatorTaxBps");
  const total = feeBps + creatorTaxBps;
  if (total >= BASIS_POINTS) invalid("totalFeeBps", "less than 10000", total);
  return total;
}

function mulDivCeil(value: bigint, multiplier: bigint, divisor: bigint): bigint {
...
function feeLeg(amount: bigint, bps: bigint, path: string): bigint {
  return checkedMul(amount, bps, path) / BASIS_POINTS;
}

function checkedMul(left: bigint, right: bigint, path: string): bigint {
...
function assertBps(value: number, path: string): void {
  if (!Number.isInteger(value) || value < 0 || value > 10_000) invalid(path, "an integer from 0 to 10000", value);
}

function assertBigintBps(value: bigint, path: string): void {
  if (value < 0n || value >= BASIS_POINTS) invalid(path, "from 0 to 9999", value);
}
...
function assertUint256(value: bigint, path: string): void {
...
function invalid(path: string, expected: string, actual: bigint | number): never {
