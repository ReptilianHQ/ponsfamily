import { decodeEventLog, getAddress, isAddress, keccak256, toBytes, type Address, type Hex, type PublicClient, type TransactionReceipt } from 'viem';
import { getV4PoolId, type V4PoolKey, type V4PoolReference } from '@reptilianhq/uniswap-sdk/v4';
import { ponsMemeHookAbi } from './abis.js';
import { assertCompatibleDeployment } from './compatibility.js';
import { getPonsDeployment, type PonsDeployment } from './deployments.js';
...
export function getPonsV4Provider(chainId = 4663) {
  ...
  return {
    schemaVersion: 1 as const, providerId: 'pons' as const, substrate: 'uniswap-v4' as const,
...
export interface PonsV4PoolReference extends V4PoolReference {
  ...
  creator: Address;
  membership: {
    sourceAddress: Address; eventSignature: typeof registrationSignature;
    blockNumber: bigint; blockHash: Hex; transactionHash: Hex; logIndex: number;
  };
}
...
function requireMatch(value: boolean, path: string): void {
  if (!value) throw new PonsSdkError('RECEIPT_FIELD_MISMATCH', 'Pons pool membership evidence does not match reviewed identity', { path });
}
 ...
 */
export function verifyPonsV4PoolRegistration(deployment: PonsDeployment, log: {
  chainId: number; address: Address; topics: readonly Hex[]; data: Hex;
  blockNumber: bigint; blockHash: Hex; transactionHash: Hex; logIndex: number; removed?: boolean;
  ...
  requireMatch(deployment.abiRevision === reviewed.abiRevision
    ...
    && (['factory', 'poolManager', 'memeHook'] as const).every(name =>
      deployment.contracts[name].toLowerCase() === reviewed.contracts[name].toLowerCase()), 'deployment');
  ...
  let decoded;
  try { decoded = decodeEventLog({ abi: ponsMemeHookAbi, topics: [...log.topics] as [Hex, ...Hex[]], data: log.data, strict: true }); }
  catch (cause) { throw new PonsSdkError('INVALID_ARGUMENT', 'Malformed Pons registration log', { cause }); }
  if (decoded.eventName !== 'PoolRegistered') throw new PonsSdkError('EVENT_NOT_FOUND', 'Expected Pons PoolRegistered');
  let poolId: Hex;
  try { poolId = getV4PoolId(key); }
  catch (cause) { throw new PonsSdkError('INVALID_ARGUMENT', 'Invalid Pons v4 pool key', { cause }); }
 ...
 */
export async function readPonsV4PoolRegistrations(client: PublicClient, transactionHash: Hex,
  options: { chainId?: number; minimumConfirmations: bigint }) {
  const deployment = getPonsDeployment(options.chainId ?? 4663);
  if (typeof options.minimumConfirmations !== 'bigint' || options.minimumConfirmations < 1n) throw new PonsSdkError('INVALID_ARGUMENT', 'An explicit positive confirmation depth is required');
  if (await client.getChainId() !== deployment.chainId) throw new PonsSdkError('CHAIN_MISMATCH', 'RPC chain differs from Pons deployment');
  const receipt: TransactionReceipt = await client.getTransactionReceipt({ hash: transactionHash });
  if (receipt.status !== 'success') throw new PonsSdkError('RECEIPT_REVERTED', 'Pons registration transaction reverted');
  requireMatch(receipt.transactionHash.toLowerCase() === transactionHash.toLowerCase(), 'transactionHash');
  const head = await client.getBlockNumber();
  if (head < receipt.blockNumber || head - receipt.blockNumber + 1n < options.minimumConfirmations) throw new PonsSdkError('CHECKPOINT_UNAVAILABLE', 'Pons registration has insufficient confirmations');
  ...
  if (!selected.length) throw new PonsSdkError('EVENT_NOT_FOUND', 'Receipt contains no reviewed Pons pool registration');
  const result: PonsV4PoolReference[] = [];
  for (const log of selected) {
    let decoded;
    try { decoded = decodeEventLog({ abi: ponsMemeHookAbi, topics: log.topics, data: log.data, strict: true }); }
    catch (cause) { throw new PonsSdkError('INVALID_ARGUMENT', 'Malformed Pons registration log', { cause }); }
    if (decoded.eventName !== 'PoolRegistered') continue;
  ...
  if ((await client.getBlock({ blockNumber: receipt.blockNumber })).hash !== receipt.blockHash) throw new PonsSdkError('CHECKPOINT_CHANGED', 'Pons registration checkpoint changed; reconcile before indexing');
  return result;
}
