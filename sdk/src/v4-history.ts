import { decodeEventLog, keccak256, parseAbi, type Address, type Hex, type PublicClient } from 'viem';
...
import { getPonsV4Provider, verifyPonsV4PoolRegistration, type PonsV4PoolReference } from './v4-provider.js';
...
function match(value: boolean, path: string): asserts value {
  if (!value) throw new PonsSdkError('RECEIPT_FIELD_MISMATCH', 'Pons history evidence does not match reviewed membership', { path });
}
const validHash = (value: unknown): value is Hex => typeof value === 'string' && /^0x[0-9a-f]{64}$/i.test(value) && !/^0x0{64}$/i.test(value);
 ...
 */
export async function readPonsV4HistoryAnchor(client: PublicClient, options: {
  chainId?: number; blockNumber: bigint; minimumConfirmations: bigint;
 ...
 */
export function verifyPonsV4HistoryMembership(anchor: PonsV4HistoryAnchor, evidence: {
  ...
  for (const log of [initialization, registration, graduation]) {
    ...
    if (log.blockNumber === anchor.blockNumber) match(log.blockHash.toLowerCase() === anchor.blockHash.toLowerCase(), 'anchor.logHash');
  }
  ...
  try {
    initialized = decodeEventLog({ abi: v4PoolManagerAbi, topics: [...initialization.topics] as [Hex, ...Hex[]], data: initialization.data, strict: true });
    graduated = decodeEventLog({ abi: ponsFactoryAbi, topics: [...graduation.topics] as [Hex, ...Hex[]], data: graduation.data, strict: true });
  } catch (cause) { throw new PonsSdkError('INVALID_ARGUMENT', 'Malformed Pons history event', { cause }); }
  if (initialized.eventName !== 'Initialize' || graduated.eventName !== 'PoolGraduated') throw new PonsSdkError('EVENT_NOT_FOUND', 'Expected Initialize and PoolGraduated');
