...
export async function readLaunchConfigs(client, deployment, options = {}) {
...
export async function readLaunchTerms(client, deployment, launcher, options = {}) {
    const blockNumber = options.blockNumber ?? await client.getBlockNumber();
...
export async function readCurveIdentityAtBlock(client, curve, options = {}) {
    ...
    const read = { address, abi: ponsCurveAbi, blockNumber };
 ...
 */
export async function readLaunchIndexingSnapshotAtBlock(client, deployment, token, launchConfigId, options = {}) {
    const blockNumber = options.blockNumber ?? await client.getBlockNumber();
    ...
    const fail = (message, path, expected, actual) => {
    ...
    if (BigInt(graduationThreshold) !== BigInt(launch.graduationThreshold)) {
        fail("Pons curve graduation threshold does not match its factory launch", "curve.graduationThreshold", launch.graduationThreshold, graduationThreshold);
    }
    if (BigInt(launch.poolFee) !== BigInt(config.poolFee) || BigInt(launch.tickSpacing) !== BigInt(config.tickSpacing)) {
        fail("Pons graduated pool parameters do not match the launch configuration", "launch.poolKey", `${config.poolFee}:${config.tickSpacing}`, `${launch.poolFee}:${launch.tickSpacing}`);
    }
    if (identity.pairToken === zeroAddress
        && (BigInt(phantomQuote) !== BigInt(config.phantomQuote)
            || BigInt(graduationThreshold) !== BigInt(config.graduationThreshold))) {
        fail("Pons native curve economics do not match the launch configuration", "curve.economics", `${config.phantomQuote}:${config.graduationThreshold}`, `${phantomQuote}:${graduationThreshold}`);
    }
    return {
        blockNumber,
        launchConfigId,
...
function requireMatch(condition, message) {
    if (!condition)
        throw new PonsSdkError("POSITION_OBSERVATION_MISMATCH", message);
}
 ...
 */
export async function readGraduatedPosition(client, deployment, token, { blockNumber }) {
    token = getAddress(token);
    ...
    if (!before.hash)
        throw new PonsSdkError('CHECKPOINT_UNAVAILABLE', 'Checkpoint block hash is unavailable');
    const launch = await readLaunchedToken(client, deployment, token, { blockNumber });
    ...
    const finish = async (result) => {
        ...
        if (after.hash !== before.hash)
            throw new PonsSdkError('CHECKPOINT_CHANGED', 'Checkpoint reorganized during the observation; rerun at a stable block');
        return { ...base, ...result };
    };
    if (launch.phase !== GraduationPhase.PoolCreated) {
        requireMatch([GraduationPhase.NotGraduated, GraduationPhase.Swept, GraduationPhase.Rescued].includes(launch.phase), 'Factory returned an unrecognized lifecycle phase');
        return finish({ status: 'no_graduated_position', lifecyclePhase: launch.phase, position: null });
    }
    ...
    requireMatch(sameAddress(lockerManager, manager) && sameAddress(lockerFactory, deployment.contracts.factory), 'Locker is not wired to the reviewed factory and V4 position manager');
    const read = { address: manager, abi: uniswapV4PositionReadAbi, args: [positionId], blockNumber };
    ...
    requireMatch(sameAddress(key.currency0, currencies[0]) && sameAddress(key.currency1, currencies[1])
        ...
        && sameAddress(key.hooks, deployment.contracts.memeHook), 'Position pool key differs from the Pons launch currencies, fee, spacing, or hook');
    const tickLower = Number(BigInt.asIntN(24, info >> 8n));
    const tickUpper = Number(BigInt.asIntN(24, info >> 32n));
    requireMatch(tickLower < tickUpper && tickLower >= -887272 && tickUpper <= 887272, 'Position tick range is invalid');
    ...
    return finish({
        ...
        position: {
            ...
            owner: getAddress(owner), locker: getAddress(locker), locked: true,
